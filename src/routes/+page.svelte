<script lang="ts">
    ...
    async function loadPlanets() {
        const { planets: newPlanets, next: newNext } =
            await planetsStore.fetchAndStorePlanets();
    ...
    async function selectPlanet(planet: Planet) {
        ...
        if (planet.id === undefined) {
            console.error('Selected planet ID is undefined.');
            return;
        }
        ...
        if (
            planet.residentsNames === null ||
            planet.residentsNames.length === 0
        ) {
            isLoadingResidents = true;

            // Fetch and display the residents with at least a 1-second delay for the loading indicator
            const updatedPlanetPromise =
                planetsStore.fetchAndStoreResidentsNames(planet.id);
            ...
            planets = planets.map((p) =>
                p.id === planet.id ? { ...p, ...updatedPlanet } : p,
            );
    ...
    <ol>
        {#each planets as planet}
            ...
            {#if selectedPlanetId === planet.id}
                ...
                {:else if planet.residentsNames && planet.residentsNames.length > 0}
                    <ul>
